...
public class ftpUtil {
     ...
     * @return
     */
    public String readFile(String pathname, String filename, String hostname, Integer port,
                           String username, String password) {
        boolean flag = false;
        String redecode = null;
        try {
            System.out.println("查询阅读文件");
            initFtpClient(hostname, port, username, password);
            // 切换FTP目录
            ftpClient.changeWorkingDirectory(pathname);
...
//            }
            ...
            ftpClient.logout();
            flag = true;
            System.out.println("阅读文件成功");
        } catch (Exception e) {
            System.out.println("阅读文件失败");
            e.printStackTrace();
        } finally {
            if (ftpClient.isConnected()) {
                try {
